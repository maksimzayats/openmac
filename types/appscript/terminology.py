...
def _makereferencetable(properties, elements, commands):
    ...
    if (
        "text" in referencebyname
    ):  # special case: AppleScript always packs 'text of...' as all-elements specifier
        referencebyname["text"] = (kElement, referencebyname["text"][1])
    for name, code, args in commands[
        ::-1
    ]:  # to handle synonyms, if two commands have same name but different codes, only the first definition should be used (iterating over the commands list in reverse ensures this)
...
def urlforapp(aemapp):
    ...
    if aemapp.AEM_identity[0] == "url":
        ...
        path = ae.addressdesctopath(
            aemapp.AEM_packself(None)
        )  # this will throw if typeMachPort; TO DO: what if current application?
...
def tablesforsdef(sdef):
    ...
    return _maketypetable(classes, enums, properties) + _makereferencetable(
...
def aetesforapp(aemapp):
    ...
    try:
        ...
        if isinstance(e, EventError) and e.errornumber == -192:
            ...
            raise RuntimeError(
                "Can't get terminology for application ({!r}): {}".format(aemapp, e)
            ) from e
    ...
    return [
        aete
        for aete in aetes
        if isinstance(aete, ae.AEDesc) and aete.type == kae.typeAETE and aete.data
    ]
...
def tablesformodule(terms):
    ...
    Result : tuple of dict -- (typebycode, typebyname, referencebycode, referencebyname)
    """
    return _maketypetable(terms.classes, terms.enums, terms.properties) + _makereferencetable(
        terms.properties, terms.elements, terms.commands
    )
