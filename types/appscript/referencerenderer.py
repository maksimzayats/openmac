...
class _Formatter:
        ...
        if nested:
            ...
            if newinstancearg:
                argstr += ", newinstance={!r}".format(
                    struct.unpack("II", self._appdata.target().addressdesc.data)
                    if self._appdata.isconnected
                    else True
                )
    ...
    def AND(self, *operands):
        self.result = "({}).AND({})".format(
            self.result, ", ".join([self._format(o) for o in operands])
        )
        return self

    def OR(self, *operands):
        self.result = "({}).OR({})".format(
            self.result, ", ".join([self._format(o) for o in operands])
        )
        return self
