...
properties = [
    (
        "class_",
        b"pcls",
    ),  # used as a key in AERecord structures that have a custom class; also, Apple have removed the 'item' class definition from skeleton.sdef
    ("properties", b"pALL"),  # Apple have removed the 'item' class definition from skeleton.sdef
    (
        "id",
        b"ID  ",
    ),  # some apps (e.g. iTunes) may omit 'id' property from terminology despite using it
]
