...
class AppData(aem.Codecs):
    ...
    def __init__(
        self, aemapplicationclass, constructor, identifier, terms, aemconstructoroptions={}
    ):
        """
    ...
    def connect(self):
        ...
        if self._terms in ["aete", "sdef", True]:  # obtain terminology from application
            ...
            "default",
            False,
        ]:  # use built-in terminology only (e.g. targeting an AppleScript applet)
        ...
        d1, d2, d3, d4 = (
            self._typebycode,
            self._typebyname,
            self._referencebycode,
            self._referencebyname,
        ) = self._terms
    ...
    def permissiontoautomate(self, code=kae.typeWildCard * 2):
        try:
            ...
            if e.args[0] == -1744:  # errAEEventWouldRequireUserConsent
                ...
                e.args[0] == -1743
            ):  # errAEEventNotPermitted (broken in 10.14.x; see <rdar://44049802>)
                return self._kNo
    ...
    def help(self, flags, ref):
        try:
            ...
            if e and e.errornumber in [
                -600,
                -609,
            ]:  # ASDictionary is no longer running, so reconnect
                if not self._inithelpagent():
                    return ref  # if ASDictionary is unavailable then do nothing
                e = self._displayhelp(flags, ref)
            if e:
...
class _Base:
    ...
    def help(
        self, flags="-t"
    ):  # add a help() method to all concrete app, reference and command objects
...
class Command(_Base):
    ...
    def __call__(self, *args, **kargs):
        ...
        if ignoreoptions is None:
            atts[b"cons"] = (
                _defaultconsiderations  # 'csig' obsoletes 'cons', but latter is retained for compatibility
            )
        ...
        try:
            return (
                self.AS_appdata
                .target()
                .event(self._code, params, atts, codecs=self.AS_appdata)
                .send(timeout, sendflags)
            )
        except aem.EventError as e:
                ...
                if not self.AS_appdata.target().processexistsforpath(self.AS_appdata.identifier):
                    if self.AS_appdata.relaunchmode == "never":
                        ...
                        if self._code == b"ascrnoop":
                            aem.Application.launch(
                                self.AS_appdata.identifier
                            )  # relaunch app in background
                        elif (
                            self._code != b"aevtoapp"
                        ):  # only 'launch' and 'run' are allowed to restart a local application that's been quit
                            raise CommandError(self, (args, kargs), e, self.AS_appdata) from e
    ...
    def AS_formatcommand(self, args):
        return "{!r}({})".format(
            self,
            ", ".join(
                ["{!r}".format(v) for v in args[0]]
                + ["{}={!r}".format(k, v) for (k, v) in args[1].items()]
            ),
        )
...
class Reference(_Base):
    ...
    def __eq__(self, val):
        return (
            ...
            and self.AS_appdata.target() == val.AS_appdata.target()
            and self.AS_aemreference == val.AS_aemreference
        )
    ...
    def __iter__(self):  # dummy-proof
        raise RuntimeError(
            "Can't iterate an application reference; use ref.get() to return a list of references first."
        )
...
class Application(Reference):
    ...
    def __init__(
        ...
        if len([i for i in [name, id, pid, url, aemapp] if i]) > 1:
            raise TypeError(
                "app() received more than one of the following arguments: name, id, pid, url, aemapp"
            )
        if name:
        ...
        Reference.__init__(
            ...
            AppData(
                ...
                identifier,
                terms,
                {"newinstance": newinstance, "hide": hide},
            ),
            aem.app,
        )
    ...
    def launch(self):
        ...
        if (
            not self.isrunning()
            and self.AS_appdata.constructor == "path"
...
app = GenericApp(
    Application
)  # app-based references are generic references unless you specify an application by calling app, e.g. app.home is generic, app('Finder').home is real
...
class CommandError(Exception):
    ...
    def __repr__(self):
        return "appscript.CommandError({!r}, {!r}, {!r})".format(
            self.command, self.parameters, self.realerror
        )
        ...
        if isinstance(self.realerror, aem.EventError):
            ...
            for label, key in [
                ...
                if key in self.realerror.raw:
                    err += "\n\t\t{}: {!r}".format(
                        label, self._codecs.unpack(self.realerror.raw[key])
                    )
        else:
            err = self.realerror
