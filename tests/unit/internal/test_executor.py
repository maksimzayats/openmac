...
def test_execute_rejects_empty_script() -> None:
...
def test_execute_raises_runtime_error_when_subprocess_fails() -> None:
    executor = AppleScriptExecutor()
...
def test_execute_raises_runtime_error_on_non_zero_exit_with_stderr() -> None:
    executor = AppleScriptExecutor()
...
def test_execute_raises_runtime_error_on_non_zero_exit_without_stderr() -> None:
    executor = AppleScriptExecutor()
    completed_process = SimpleNamespace(
        returncode=1,
        stdout="",
...
def test_execute_returns_stdout_without_trailing_newline() -> None:
    executor = AppleScriptExecutor()
    completed_process = SimpleNamespace(
        returncode=0,
        stdout="ok\n",
